...
void *reader(void *param) {
   ...
   for(count=0; count<4 ; count++) {
      while(__sync_lock_test_and_set(&writer_lock, 1));   //function which returns the initial value of variable lock
      ...
      __sync_lock_release(&writer_lock);
      sleep(rand()%3+1);
      sem_post(&empty);
